...
import { useProductsByBrand } from '../../Hooks/shared-queries';
...
export default function Brands() {
  ...
  // Use the shared query for products by brand
  const { data, isLoading, isError } = useProductsByBrand(brandId);
  ...
  useEffect(() => {
    if (isError || (data && data.results === 0)) {
      navigate('/brands');
    }
  ...
  const pageCount = data?.data?.data ? Math.ceil(data.data.data.length / itemsPerPage) : 0;
  ...
  return (
    <>
      {isLoading ? (
        ...
        <p className="text-red-500 text-center">Error loading products. Please try again later.</p>
      ) : (
        <div className="flex flex-col items-center mt-10">
          {/* Brand title if available */}
          {data?.data?.data?.[0]?.brand?.name && (
            <h2 className="text-2xl font-bold mb-6">{data.data.data[0].brand.name} Products</h2>
          )}
